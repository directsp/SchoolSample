...
BEGIN
	...
	EXEC dspUtil.LogTrace @ProcId = @@PROCID, @Message = N'Getting Stored Procedures definition';
	...
	BEGIN
		...
		EXEC dspUtil.LogTrace @ProcId = @@PROCID, @Message = N'Removing Space, Tab, line feed';
		...
		EXEC dspUtil.LogTrace @ProcId = @@PROCID, @Message = N'Cutting out text before /*con st';
		...
		EXEC dspUtil.LogTrace @ProcId = @@PROCID, @Message = N'Validate Function Id with Corresponding value';
		...
		BEGIN
			...
			DECLARE @IsMatch BIT;
			EXEC dspUtil.CompareConstFunctionReturnValueWithScriptValue @Script = @Definition OUTPUT, @ConstFunctionName = @ConstFunctionName OUTPUT,
			...
			BEGIN
				...
				EXEC @Message = dspUtil.FormatMessage @Message = N'ConstValueInFunction({0}) and ConstValueInScript({1}) are inconsistence; the function name is: {2} in the SP: {3}',
