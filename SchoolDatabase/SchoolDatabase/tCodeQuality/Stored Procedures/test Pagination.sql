...
BEGIN
	-- Declaring pattern
	DECLARE @Pattern_Offset TSTRING = dspUtil.String_RemoveWhitespaces('OFFSET @RecordIndex ROWS FETCH NEXT @RecordCount ROWS ONLY');
	DECLARE @Pattern_PageIndex TSTRING = dspUtil.String_RemoveWhitespaces('@RecordIndex = @RecordIndex');
	...
	EXEC dspUtil.LogTrace @ProcId = @@PROCID, @Message = N'Checking implementation paging in api and dbo StoreProcedure';
