...
CREATE PROCEDURE [dspUtil].[InitTemporalTable]
...
BEGIN
	...
	BEGIN
		...
		EXEC dspUtil.LogTrace @ProcId = @@PROCID, @Message = N'Trying to SET the SYSTEM_VERSIONING OFF. Table: {0}', @Param0 = @TableName;
		BEGIN TRY
			EXEC ('ALTER TABLE ' + @Schema + '.' + @TableName + ' SET (SYSTEM_VERSIONING = OFF)');
		END TRY
		BEGIN CATCH
			SET @Msg = ERROR_MESSAGE();
			EXEC dspUtil.LogError @ProcId = @@PROCID, @Message = @Msg;
		END CATCH;

		--  Drop PERIOD life time
		EXEC dspUtil.LogTrace @ProcId = @@PROCID, @Message = N'Dropping PERIOD FOR SYSTEM_TIME; Table: {0}', @Param0 = @TableName;
		BEGIN TRY
